...
CKEDITOR.plugins.add('wpmore', {
    ...
    getPlaceholderCss : function () {
        return 'img.cke_wordpress_more'
             + '{'
                 + 'background-image: url(' + CKEDITOR.getUrl(this.path + 'img/more_bug.gif') + ');'
    ...
    init: function (editor) {
        ...
        editor.ui.addButton('WPMore', {
            label: 'Wstaw Czytaj więcej',
            icon: this.path + 'img/more.gif',
            command: 'wpmore'
        });
            ...
            exec: function () {
                ...
                for (var i = 0, len = images.count() ; i < len ; i++ ) {
                    ...
                    if (img.hasClass('cke_wordpress_more')) {
                        var msg = 'Ten dokument zawiera Czytaj więcej. '
                                + 'Czy kontynuować z próbą usunięcia?'
                        if (confirm(msg)) {
                            img.remove();
                            break;
                        } else {
